...
export class UserService {
  ...
  public login(user: LoginRM): Observable<IToken> {
    return this.http.post<ApiResult<IToken>>(`${this.baseUrl}Clients/Login`, user)
      .pipe(
         switchMap(r => r.isSuccess ? of(r.value as IToken) : throwError(() => new Error(r.error || 'Operation failed'))),
        catchError(this.errorService.handleError.bind(this.errorService))
      );;
  }

  public register(user: RegisterRM): Observable<IToken> {
    return this.http.post<ApiResult<IToken>>(`${this.baseUrl}Clients/Register`, user)
      .pipe(
         switchMap(r => r.isSuccess ? of(r.value as IToken) : throwError(() => new Error(r.error || 'Operation failed'))),
        catchError(this.errorService.handleError.bind(this.errorService))
      );
